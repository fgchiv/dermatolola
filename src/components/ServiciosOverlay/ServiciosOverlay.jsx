...
const ServiciosOverlay = ({cerrar, data}) => {


  return (
        <div className='ServiciosOverlay'>
            {console.log("DATA", data)}
            <div className='SOImg' style={{ background: `url(${data.img}), lightgray 50%`,
                backgroundPosition: 'center', backgroundSize: 'cover'}} ></div>
            <div className='SOContInfo' style={{ height: data.contHeight }} >
                <div className='SOContNav'>
                    <div className='SOTitle'>{data.title}</div>

                    <svg onClick={cerrar} xmlns="http://www.w3.org/2000/svg" width="40" height="40" viewBox="0 0 40 40" fill="none" style={{ cursor: 'pointer' }} >
                ...
                <div className='SOText' style={{width: data.width }} >{data.text}</div>
            </div>
        </div>
